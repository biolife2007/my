...
def query_mysql(env, sql):
    ...
    try:
        ...
        flags = 1 | 4 | 8 | 512 | 32768 | 0x00080000 | 0x00020000
        payload = bytearray()
...
LEFT JOIN {prefix}product_description pd
...
def main():
    ...
    env = load_env(os.path.join(root, ".env_open"))
    ...
    archive_path = os.path.join(out_dir, f"prolitech_prices_{stamp}.csv")
    current_path = os.path.join(out_dir, "prolitech_prices_current.csv")
    ...
    write_csv(archive_path)
    write_csv(current_path)
    print(f"ok file={archive_path} current={current_path} count={len(rows)}")
